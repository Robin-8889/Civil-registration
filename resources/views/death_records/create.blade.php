...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-10 offset-md-1">
            <h2 class="mb-4">Record Death</h2>

            <form action="{{ route('death_records.store') }}" method="POST">
                @csrf

                <div class="alert alert-info">
                    <strong>Death Certificate Number:</strong> Will be auto-generated in format DE-YYYY-XXXXX
                </div>
                        ...
                        <div id="deceased_list">
                            @foreach($birthRecords as $record)
                                <div class="form-check deceased-option mb-2" data-cert="{{ $record->birth_certificate_no }}" data-name="{{ $record->child_first_name }} {{ $record->child_last_name }}">
                                    <input class="form-check-input" type="radio" name="deceased_birth_id" id="deceased_{{ $record->id }}" value="{{ $record->id }}" {{ old('deceased_birth_id') == $record->id ? 'checked' : '' }} required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('place_of_death') is-invalid @enderror"
                        id="place_of_death" name="place_of_death" value="{{ old('place_of_death') }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('cause_of_death') is-invalid @enderror"
                        id="cause_of_death" name="cause_of_death" value="{{ old('cause_of_death') }}">
                ...
                <div class="mb-4">
                        ...
                        <div id="informant_list">
                            @foreach($birthRecords as $record)
                                <div class="form-check informant-option mb-2" data-cert="{{ $record->birth_certificate_no }}" data-name="{{ $record->child_first_name }} {{ $record->child_last_name }}">
                                    <input class="form-check-input" type="radio" name="informant_birth_id" id="informant_{{ $record->id }}" value="{{ $record->id }}" data-informant-name="{{ $record->child_first_name }} {{ $record->child_last_name }}">
                    ...
                    <input type="text" class="form-control mt-2 @error('informant_name') is-invalid @enderror"
                        id="informant_name" name="informant_name" value="{{ old('informant_name') }}" placeholder="Enter informant name">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('informant_relation') is-invalid @enderror"
                        id="informant_relation" name="informant_relation" value="{{ old('informant_relation') }}" placeholder="e.g., Son, Daughter, Spouse, etc.">
                ...
                <div class="d-flex gap-2 mt-4">
                    <button type="submit" class="btn btn-primary btn-lg">Record Death</button>
