...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-10 offset-md-1">
            <h2 class="mb-4">Edit Marriage Record</h2>
            ...
            <form action="{{ route('marriage_records.update', $marriageRecord) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="alert alert-info">
                    <strong>Marriage Certificate Number:</strong> {{ $marriageRecord->marriage_certificate_no }}
                </div>
                        ...
                        <div id="groom_list">
                            @foreach($grooms as $record)
                                <div class="form-check groom-option mb-2" data-cert="{{ $record->birth_certificate_no }}" data-name="{{ $record->child_first_name }} {{ $record->child_last_name }}">
                                    <input class="form-check-input" type="radio" name="groom_id" id="groom_{{ $record->id }}" value="{{ $record->id }}" {{ old('groom_id', $marriageRecord->groom_id) == $record->id ? 'checked' : '' }}>
                        ...
                        <div id="bride_list">
                            @foreach($brides as $record)
                                <div class="form-check bride-option mb-2" data-cert="{{ $record->birth_certificate_no }}" data-name="{{ $record->child_first_name }} {{ $record->child_last_name }}">
                                    <input class="form-check-input" type="radio" name="bride_id" id="bride_{{ $record->id }}" value="{{ $record->id }}" {{ old('bride_id', $marriageRecord->bride_id) == $record->id ? 'checked' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('date_of_marriage') is-invalid @enderror"
                        id="date_of_marriage" name="date_of_marriage" value="{{ old('date_of_marriage', $marriageRecord->date_of_marriage ? \Carbon\Carbon::parse($marriageRecord->date_of_marriage)->format('Y-m-d') : '') }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('place_of_marriage') is-invalid @enderror"
                        id="place_of_marriage" name="place_of_marriage" value="{{ old('place_of_marriage', $marriageRecord->place_of_marriage) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('registration_office_id') is-invalid @enderror"
                        ...
                        @foreach($offices as $office)
                            <option value="{{ $office->id }}" {{ old('registration_office_id', $marriageRecord->registration_office_id) == $office->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                        <option value="registered" {{ old('status', $marriageRecord->status) == 'registered' ? 'selected' : '' }}>Registered</option>
                        <option value="pending" {{ old('status', $marriageRecord->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="rejected" {{ old('status', $marriageRecord->status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-4">
                        ...
                        <div id="witness1_list">
                            @foreach($birthRecords as $record)
                                <div class="form-check witness1-option mb-2" data-cert="{{ $record->birth_certificate_no }}" data-name="{{ $record->child_first_name }} {{ $record->child_last_name }}">
                                    <input class="form-check-input" type="radio" name="witness1_birth_id" id="witness1_{{ $record->id }}" value="{{ $record->child_first_name }} {{ $record->child_last_name }}" {{ old('witness1_birth_id') == ($record->child_first_name . ' ' . $record->child_last_name) || $marriageRecord->witness1_name == ($record->child_first_name . ' ' . $record->child_last_name) ? 'checked' : '' }}>
                    ...
                    <input type="hidden" id="witness1_name" name="witness1_name" value="{{ old('witness1_name', $marriageRecord->witness1_name) }}">
                </div>
                ...
                <div class="mb-4">
                        ...
                        <div id="witness2_list">
                            @foreach($birthRecords as $record)
                                <div class="form-check witness2-option mb-2" data-cert="{{ $record->birth_certificate_no }}" data-name="{{ $record->child_first_name }} {{ $record->child_last_name }}">
                                    <input class="form-check-input" type="radio" name="witness2_birth_id" id="witness2_{{ $record->id }}" value="{{ $record->child_first_name }} {{ $record->child_last_name }}" {{ old('witness2_birth_id') == ($record->child_first_name . ' ' . $record->child_last_name) || $marriageRecord->witness2_name == ($record->child_first_name . ' ' . $record->child_last_name) ? 'checked' : '' }}>
                    ...
                    <input type="hidden" id="witness2_name" name="witness2_name" value="{{ old('witness2_name', $marriageRecord->witness2_name) }}">
                </div>

                <div class="d-flex gap-2 mt-4">
                    <button type="submit" class="btn btn-primary btn-lg">Update Marriage Record</button>
                    <a href="{{ route('marriage_records.show', $marriageRecord) }}" class="btn btn-secondary btn-lg">Cancel</a>
                </div>
            </form>
        </div>
    </div>
